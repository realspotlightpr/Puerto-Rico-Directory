  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" role="dialog" aria-modal="true" aria-labelledby="claim-title">
      ...
      <div className="relative bg-white w-full sm:max-w-lg sm:mx-4 rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden max-h-[94vh] overflow-y-auto">
        ...
        <div className="p-6">
          {done ? <div className="py-5 text-center"><CheckCircle2 className="w-14 h-14 text-emerald-500 mx-auto mb-4" /><h3 className="text-xl font-bold">Claim received</h3><p className="text-sm text-muted-foreground mt-2 max-w-sm mx-auto">Your claim is now in review. If verification is needed, our team will contact <strong>{email}</strong>{phone.trim() ? " or your mobile number" : ""}.</p><div className="mt-5 rounded-xl bg-slate-50 p-4 text-left text-sm"><p className="font-semibold">What happens next</p><ol className="mt-2 space-y-1 text-muted-foreground list-decimal pl-5"><li>Complete any verification request you receive.</li><li>Our team reviews the business connection.</li><li>Approved owners receive dashboard access.</li></ol></div><Button className="mt-6 w-full h-12" onClick={onClose}>Done</Button></div> :
