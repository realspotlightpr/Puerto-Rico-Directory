...
import { useGetBusiness, useListBusinessReviews, useCreateReview, useClaimBusiness, useListBusinesses } from "@workspace/api-client-react";
        ...
        <div className="mt-3 space-y-1.5">
          {DAYS.map(day => (
            ...
            >
              ...
              <span className={`${day === today ? "text-foreground" : "text-foreground"} ${hours[day]?.toLowerCase() === "closed" || !hours[day] ? "text-muted-foreground italic" : ""}`}>
                {hours[day] || "Closed"}
              </span>
            </div>
          ))}
...
function InquiryForm({ businessId, businessName }: { businessId: number; businessName: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSending(true);
    try {
      const resp = await fetch(`${API_BASE}api/businesses/${businessId}/inquiry`, {
        method: "POST",
...
// ─── Similar businesses ────────────────────────────────────────────────────────
function SimilarBusinesses({ businessId, categoryId, municipality }: { businessId: number; categoryId?: number | null; municipality?: string | null }) {
  const { data } = useListBusinesses(
    categoryId ? { category: categoryId.toString(), limit: 5 } : { municipality: municipality || "", limit: 5 }
  );

  const items = (data?.businesses || []).filter(b => b.id !== businessId).slice(0, 4);
  ...
  return (
    <div className="mt-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {items.map(b => (
          <Link key={b.id} href={`/businesses/${(b as any).slug || b.id}`}>
            <div className="bg-card rounded-2xl border border-border shadow-sm hover:shadow-md transition-shadow overflow-hidden cursor-pointer group">
              <div className="h-28 bg-muted relative overflow-hidden">
                {b.coverUrl ? (
                  <img src={b.coverUrl} alt={b.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                ) : (
                  <div className="w-full h-full bg-gradient-to-br from-primary/10 to-primary/5 flex items-center justify-center">
                    <span className="text-4xl font-bold text-primary/20 font-display">{b.name[0]}</span>
                  </div>
                )}
                {b.logoUrl && (
              ...
              <div className="p-3">
                <p className="font-semibold text-sm text-foreground leading-tight line-clamp-1 group-hover:text-primary transition-colors">{b.name}</p>
                <p className="text-xs text-muted-foreground mt-0.5 flex items-center gap-1">
                  <MapPin className="w-3 h-3" /> {b.municipality}
                </p>
                {(b.averageRating ?? 0) > 0 && (
                  <div className="flex items-center gap-1 mt-1">
                    <Star className="w-3 h-3 fill-amber-400 text-amber-400" />
                    <span className="text-xs font-bold text-foreground">{b.averageRating!.toFixed(1)}</span>
...
export default function BusinessDetail() {
  ...
  const publicUrl = `${window.location.origin}/businesses/${(business as any).slug || business.id}`;
    ...
    <div className="bg-gray-50 min-h-screen pb-24">
        ...
        {(business as any).specialOffer && (
          <div className="mb-8 bg-gradient-to-r from-amber-50 to-teal-50 border border-amber-200 rounded-2xl p-5 flex items-start gap-4 shadow-sm">
            ...
            <div>
              ...
              <p className="text-foreground font-semibold">{(business as any).specialOffer}</p>
            </div>
          </div>
        )}
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 lg:hidden bg-white border-t border-border shadow-2xl px-4 py-2 safe-area-bottom">
        <div className="flex gap-2 max-w-xl mx-auto">
          ...
          {business.address && (
            <a
              href={`https://maps.google.com/?q=${encodeURIComponent([business.address, business.municipality, "Puerto Rico"].filter(Boolean).join(", "))}`}
              target="_blank"
              rel="noopener noreferrer"
              className="flex-1"
              onClick={() => {
